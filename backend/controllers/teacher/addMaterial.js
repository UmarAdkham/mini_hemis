...
exports.getAllMaterials = async (req, res) => {
  try {
    ...
    res.status(200).json({messege: "Materiallar muvaffaqiyatli olindi", data: result.rows});
  } catch (error) {
    console.log(error);
...
exports.getMaterialById = async (req, res) => {
  try {
    ...
    const result = await pool.query(`select id, title, filepath from courses
      inner join materials on courses.id = materials.course_id
      where materials.course_id = $1`, [course_id]);
    res.status(200).json({messege: "Materialla muvaffaqiyatli olindi", data: result.rows});
  } catch (error) {
    console.log(error);
